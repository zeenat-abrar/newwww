...
import { platformMenuItems } from "@/components/blocks/mega-menu";
...
export const PlatformMenu = ({ closeMenu }: PlatformMenuProps) => {
  const [hoveredIndex, setHoveredIndex] = useState<number | null>(null);
  return (
    <div className="grid w-[840px] grid-cols-[135px_135px_135px_135px_133px_135px] px-4">
      <div className="col-span-6 py-3">
        <h2 className="font-secondary text-xl font-semibold">
          A <span className="text-ivviolet-600">Unified Data Platform</span> for
        </h2>
      </div>
      ...
      {platformMenuItems.map(({ icon, label, href }, index) => (
        <Link
          key={label}
          href={href}
