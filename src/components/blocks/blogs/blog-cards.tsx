...
export interface blog {
  ...
  href: string;
  headingHeightClassName?: string;
}

interface BlogCradsProps {
  heading?: string;
  data: blog[];
}

const BlogCards = ({ data, heading }: BlogCradsProps) => {
  return (
            ...
            <CardContainer key={card.id} className="size-full">
              <CardBody className={cn("size-auto px-2 py-6", "bg-opacity-25")}>
                <CardItem
                  translateZ="50"
                  className={cn(
                    "md:min-h-[85px] lg:min-h-[140px] xl:min-h-[100px] 2xl:min-h-[85px] w-full text-center text-xl font-semibold text-black px-4",
                    card.headingHeightClassName
                  )}
                >
                  {card.title}
                </CardItem>
                ...
                <CardItem as="p" translateZ="50" className="w-full text-center text-sm max-w-sm pt-5 px-4 3xl:px-2 mx-auto">
                  {card.description}
                </CardItem>
              </CardBody>
            </CardContainer>
