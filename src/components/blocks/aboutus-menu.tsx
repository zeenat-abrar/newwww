...
import { aboutUsMenuItems } from "@/components/blocks/mega-menu";
...
export const AboutUsMenu = ({ closeMenu }: AboutUsMenuProps) => {
  const [hoveredIndex, setHoveredIndex] = useState<number | null>(null);
  return (
    <div className="grid w-[840px] grid-cols-[135px_135px_135px_135px_135px_135px] px-4">
      <div className="col-span-6 py-3">
        <h2 className="font-secondary text-xl font-semibold">About Infoveave</h2>
      </div>
      ...
      {aboutUsMenuItems.map(({ icon, label, href }, index) => (
        <Link
          key={label}
          href={href}
        ...
        >
          {label == "Brand" ? (
            <Image src={InfoveaveFlower} alt="infoveave-logo" width={80} height={80} className="mt-[-5px] mb-[-3px] ml-9" />
          ) : (
